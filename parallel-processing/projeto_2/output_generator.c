...
int recordGenerator (tRecord *r, size_t qtRecord, size_t lsize, size_t csize, size_t qtProcess, FILE *f_out)
{
    ...
    fprintf(f_out, "| N Processor(s) | %*s%*s | %*s%*s | Maior Diff |   Speedup  | Eficiencia |\n",
            7, "min", 3, " ", 7, "max", 3, " ");
    fprintf(f_out, "+----------------+------------+------------+------------+------------+------------+\n");
    for (int i = 0; i < qtRecord; i++)
    {
        speedup = t_sq/r[i].menor_tempo;
        fprintf(f_out, "| %14d | %*f | %*f | %*f | %*f | %*f |\n", r[i].num_processors, 10, r[i].menor_tempo,
                10, r[i].maior_tempo, 10 , r[i].diff, 10, speedup, 10, speedup/r[i].num_processors);
    }
    fprintf(f_out, "+----------------+------------+------------+------------+------------+------------+\n");
